/*
  Modeling following pseudocode describing each philosopher's behaivour:
    ...
    forever repeat
    {
      ...
      t, ticket := ticket, ticket + 1;   // Request to enter kitchen
      Hungry:
      
      wait until serving = t;            // Enter kitchen
      Eating:
      // ...
    }

  The "wait until" operation is modeled as Busy Waiting.
...
class TicketSystem
{
  ...
  // TryEnter models "wait until serving = t" as Busy waiting
  // If its our turn we can enter, but on the contrary we skip and try again later
  method TryEnter(p: Process)
    modifies this  
    requires Inv() && p in P && cs[p] == Hungry
    ensures  Inv()
  {
    if t[p] == serving {       
      cs := cs[p := Eating];
